...
const BASE_PATH = location.pathname.startsWith('/RaymondWrite/playwright-toolkit') ? '/RaymondWrite/playwright-toolkit' : '';
...
function routeFromLocation() {
  ...
  $$('.nav-links a').forEach((link) => {
    ...
    link.classList.toggle('active', href === path || (isChatAnchor && href === '/#chat'));
  });
...
function communityCard(item) {
  ...
  return `<article class="community-card" data-id="${item.id}"><div class="script-top"><span class="script-status">${esc(item.framework_type || '未分类')}</span><span>${formatDate(item.created_at)}</span></div><h3>${esc(item.title)}</h3><p class="script-summary">${esc(item.content).slice(0, 120)}${item.content.length > 120 ? '…' : ''}</p><div class="tag-row">${(item.tags || []).map((tag) => `<span>${esc(tag)}</span>`).join('')}</div><div class="community-meta"><button class="mini-link" onclick="window.appNavigate('/script/${item.id}')">查看全文</button><a href="/profile/${item.user_id}" data-route>作者：${esc(author)}</a><span>❤ ${item.likes_count || 0}</span></div></article>`;
}
